    ...
    def __new__(cls, *args, **kwargs):
        if cls is Opcode:
            ...
            if info.name in OPCODES_BY_NAME:
                del kwargs["info"]
                # print(info.code.encode().hex())
                print(hex(ord(info.code)),info.name)
                # print(OPCODES_BY_NAME[info.name])
                return OPCODES_BY_NAME[info.name](*args, **kwargs)
            else:
                print("err",hex(ord(info.code)))
...
class StackGlobal(NoOp):
    ...
    def run(self, interpreter: Interpreter):
        ...
        if module == "__builtin__":
            ...
            interpreter.module_body.append(
                # ast.ImportFrom(module=module, names=[alias], level=0)
                ast.ImportFrom(module=module.id, names=[alias.name], level=0)
            )
        #print(module.id)
        #print(attr.value)
        interpreter.stack.append(ast.Name(attr, ast.Load()))
...
class Frozz(Opcode):
    name = "FROZENSET"
    def run(self, interpreter: Interpreter):
        tmp = ast.List([], ast.Load())
        while interpreter.stack:
            obj = interpreter.stack.pop()
            if isinstance(obj, MarkObject):
                break
            else:
                if isinstance(tmp, ast.List):
                    tmp.elts.append(obj)
        interpreter.stack.append(tmp)
